...
// Channels pulled into News regardless of their A1X group
...
// Non-English / unwanted channels excluded entirely
...
// Map AU IPTV channel IDs -> exact A1X M3U channel name (for backup stream handler)
...
function parseM3U(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const tvgLogo = (lines[i].match(/tvg-logo="([^"]+)"/i) || [])[1] || ''
    ...
    if (url && group && name) entries.push({ group, name, logo: tvgLogo, url })
  }
  return entries
}
...
async function getCache() {
  ...
  for (const cat of CATEGORIES) {
    ...
    byCategory[cat] = byCategory[cat].filter(ch => {
      ...
      seen.add(ch.id); return true
    })
  }

  // Flat lookup by id and by name
  const allEntries = {}
  const byName = {}
  for (const cat of CATEGORIES) {
...
builder.defineCatalogHandler(async ({ type, id }) => {
  ...
  const catMap = { 'a1x-sports': 'Sports', 'a1x-entertainment': 'Entertainment', 'a1x-news': 'News' }
  const category = catMap[id]
  ...
  } catch (err) { console.error('Catalog error:', err.message); return { metas: [] } }
})
...
builder.defineMetaHandler(async ({ id }) => {
  ...
  // au| IDs — return basic meta so Stremio shows the channel
