...
async function fetchBrowse(url, append) {
  ...
  if (!append) {
    grid.innerHTML = '<div class="empty" style="grid-column:1/-1"><div class="empty-icon"><span class="material-icons" style="font-size:48px;color:var(--color-text-disabled);">hourglass_empty</span></div><div class="empty-text">Loading…</div></div>';
  } else {
  ...
  try {
    ...
    if (!append) grid.innerHTML = `<div class="empty" style="grid-column:1/-1"><div class="empty-icon"><span class="material-icons" style="font-size:48px;color:var(--color-error);">error</span></div><div class="empty-text">${esc(e.message)}</div></div>`;
...
function renderMovieGrid(hasMore = false) {
  const grid = document.getElementById('movie-grid');
  if (!movieResults.length) { grid.innerHTML = '<div class="empty" style="grid-column:1/-1"><div class="empty-icon"><span class="material-icons" style="font-size:48px;color:var(--color-text-disabled);">inbox</span></div><div class="empty-text">No results.</div></div>'; return; }
